...
for c in "${CONFIGS[@]}"; do
  ...
  while read -r q s _; do
    ...
    # Pooled detect hard-fails ("no usable confidence levels") on pairs where
    # every level is dropped by the median-distinguishability screen (typical
    # for very distant pairs); log and continue instead of aborting the run.
    # shellcheck disable=SC2086
    "$GDIFF" --num-threads "$THREADS" detect "$(fa "$q")" "$CACHE/$name/$s.gdiff" \
      $det_args -o "$out" 2>/dev/null \
      || echo "  detect failed: $q vs $s (skipped)" >&2
